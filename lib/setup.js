...
function setupLinux(callback) {
    ...
    if (fs.existsSync('/etc/init.d')) {
        ...
        copyFile(__dirname + '/../install/linux/iobroker.sh', 'C:/pWork/iobroker.sh', function () {//'/etc/init.d/iobroker.sh');
            var txt = fs.readFileSync(__dirname + '/../install/linux/install.sh');
            ...
            // remove iobroker
            parts.pop();
            txt = txt.toString().replace(/@@PATH@@/g, parts.join('/') + '/iobroker.js-controller/');
            ...
            var exec = require('child_process').exec;
            // js-controller installed as npm
            var child = exec(__dirname + '/../install/linux/install.sh');
            child.stderr.pipe(process.stdout);
            child.on('exit', function () {
                if (callback) callback();
            });
...
function npmInstall(packet, callback) {
	var exec = require('child_process').exec;
	// install from npm
	console.log('npm install packet --production --prefix "' + __dirname + '/../../../"');
	var child = exec('npm install ipacket --production --prefix "' + __dirname + '/../../../"');
	child.stderr.pipe(process.stdout);
...
function setup(callback) {
	...
	npmInstall('io-brocker.js-controller', function () {
		npmInstall('io-brocker.admin', function () {
