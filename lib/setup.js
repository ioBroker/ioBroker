...
function setupLinux(callback) {
    ...
    try{
        ...
        console.log('You can manually copy file into /usr/bin/. Just write "sudo cp ' + path.normalize(__dirname + '/../install/iobroker') + ' /usr/bin/');
    ...
    if (fs.existsSync('/etc/init.d')) {
        ...
        copyFile(__dirname + '/../install/linux/iobroker.sh', '/etc/init.d/iobroker.sh', function (err) {
            ...
            if (err) {
                ...
                console.log('You can manually copy file: sudo cp ' + path.normalize(__dirname + '/../install/linux/iobroker.sh') + ' /etc/init.d/')
                console.log('chmod 777 /etc/init.d/iobroker.sh');
...
function npmInstall(packet, isSudo, callback) {
    ...
    if (platform == 'linux' || platform == 'darwin') {
        cmd = 'npm install ' + packet + ' --production --prefix ' + path;
	...
	child.on('exit', function () {
        // Check if exist
        if ((platform == 'linux' || platform == 'darwin') && !isSudo && !fs.existsSync(path + '/node_modules/' + packet)) {
