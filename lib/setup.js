		...
		npmInstall('iobroker.admin', false, function () {
			npmInstall('iobroker.js-controller', false, function () {
                ...
                try {
                    ...
                    var jsVersion = JSON.parse(fs.readFileSync(rootDir + 'node_modules/iobroker.js-controller/package.json').toString()).version;
                    var discoveryVersion = JSON.parse(fs.readFileSync(rootDir + 'node_modules/iobroker.discovery/package.json').toString()).version;
                    var adminVersion = JSON.parse(fs.readFileSync(rootDir + 'node_modules/iobroker.admin/package.json').toString()).version;
                    pack.dependencies['iobroker.discovery']     = '~' + discoveryVersion;
                    pack.dependencies['iobroker.admin']         = '~' + adminVersion;
                    pack.dependencies['iobroker.js-controller'] = '~' + jsVersion;
                    fs.writeFileSync(rootDir + 'package.json', JSON.stringify(pack, null, 2));
                } catch (e) {
                    console.error('Cannot update ' + rootDir + 'package.json');
                }
