...
function setupWindows(callback) {
    ...
    const files = fs.readdirSync(__dirname + '/../install/windows');
    let cnt = 0;
    for (let f = 0; f < files.length; f++) {
        cnt++;
        copyFile(__dirname + '/../install/windows/' + files[f], rootDir + files[f], () => {
            if (!--cnt) {
                ...
                const child = exec('npm install https://github.com/arthurblake/node-windows/tarball/f1fd60e93e2469663b99a9cf3a64086ecedfe1e4 --production --save --prefix "' + npmRootDir + '"');
...
function setupFreeBSD(callback) {
    ...
    if (fs.existsSync('/usr/local/etc/rc.d')) {
        ...
        let txt = fs.readFileSync(__dirname + '/../install/freebsd/iobroker');
        ...
        copyFile(__dirname + '/../install/freebsd/iobroker', '/usr/local/etc/rc.d/iobroker', err => {
            ...
            if (err) {
                ...
                child.on('exit', errCode => {
                    console.log('Auto-start was enabled. Write "sysrc iobroker_enable=NO" to disable auto-start');
...
function setupLinux(callback) {
    ...
    if (fs.existsSync('/etc/init.d')) {
        ...
        let txt = fs.readFileSync(__dirname + '/../install/linux/iobroker.sh');
        txt = txt.toString().replace(/@@PATH@@/g, _path);
        txt = txt.toString().replace(/@@HOME@@/g, home.join('/'));
        ...
        copyFile(__dirname + '/../install/linux/iobroker.sh', '/etc/init.d/iobroker.sh', err => {
