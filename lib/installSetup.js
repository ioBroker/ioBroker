...
function setupWindows(callback) {
    ...
    const child = exec('npm install node-windows@0.1.14 --production --save --prefix "' + npmRootDir + '"', (error, stdout, stderr) => {
        ...
        console.log('Windows service library installed, not register ioBroker as Service');
        // call node install.js
        // install node as service
        console.log('node "' + path.join(rootDir, 'install.js') + '"');
        const child1 = exec('node "' + path.join(rootDir, 'install.js') + '"', (error, stdout, stderr) => {
            console.log('Error: ' + error);
            console.log('Stderr: ' + stderr);
            /*if (error) {
                if (callback) callback(error.code);
