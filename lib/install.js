...
let isNpmInstall;
if (process.env.npm_config_argv) { // npm <= 6
    const npmArgs = JSON.parse(process.env.npm_config_argv);
    isNpmInstall = npmArgs && npmArgs.cooked && (npmArgs.cooked.indexOf('install') > -1 || npmArgs.cooked.indexOf('i') > -1)
} else if (process.env.npm_command === 'install' || process.env.npm_command === 'i') { // npm 7+
    isNpmInstall = true;
}
if (!isNpmInstall) {
    // This script is not run as part of an installation (e.g. rebuild), so don't install!
    console.error(colors.yellow('lib/install.js is not being run as part of an installation - skipping...'));
    process.exit(0);
}
...
if (tools.isThisInsideNodeModules()) {
    process.stderr.write("Executing installCopyFiles\n");
