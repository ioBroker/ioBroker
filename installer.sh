...
create_user_linux() {
	...
	SUDOERS_CONTENT=$(cat <<- EOF
		...
		$username	ALL=(ALL)	NOPASSWD: /sbin/shutdown -h now, /sbin/halt, /sbin/poweroff, /sbin/reboot
		$username	ALL=(ALL)	NOPASSWD: /bin/systemctl start, /bin/systemctl stop
		$username	ALL=(ALL)	NOPASSWD: /bin/mount -o nosuid\,nodev\,noexec, /bin/mount, /bin/umount
		$username	ALL=(ALL)	NOPASSWD: /usr/bin/apt-get, /usr/bin/apt, /usr/bin/dpkg, /usr/bin/make
		$username	ALL=(ALL)	NOPASSWD: /bin/ping, /usr/bin/ping, /usr/sbin/ping, /usr/bin/fping, /usr/sbin/fping, /usr/bin/arp-scan
